...
object RateRepository {
    ...
    // Your deployed server. Use http://10.0.2.2:3000 to test on the emulator.
    private const val ENDPOINT = "https://your-server.example.com/api/rate"
    ...
    fun cached(context: Context): Rate? {
        ...
        return Rate(
            ...
            silver999PerKg = p.getFloat(KEY_SILVER_KG, 0f).toDouble(),
            fetchedAt = at
        )
    }
    ...
    suspend fun refresh(context: Context): Rate? = withContext(Dispatchers.IO) {
        try {
            ...
            val silver = json.getJSONObject("silver")

            val rate = Rate(
                gold999Per10g = gold.getDouble("per10g999"),
                gold916Per10g = gold.getDouble("per10g916"),
                silver999PerKg = silver.getDouble("perKg999"),
            ...
            context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().apply {
                ...
                putFloat(KEY_SILVER_KG, rate.silver999PerKg.toFloat())
